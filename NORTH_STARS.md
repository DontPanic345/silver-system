...
## 4. Gnomes — the actual game (2026-09-05)

> I thought building a physics sim was a cool endeavour on its own. And with
> all these problems with [Oxygen Not Included], I thought surely we could
> smash them together and build something cool.

The fullest capstone so far, and the one everything above has actually been
in service of: a specific game, not just "an emergent physics sim in the
spirit of ONI." It opens as a direct list of what ONI got wrong and this
project exists to fix:

- No real mass or energy conservation — "broke immersion."
- Liquids flow *up* through pipes in a layer disconnected from the physical
  world — gravity-defying and it shows.
- Gas behaves nothing like gas (CO2 doesn't actually settle the way ONI's
  simplified layers show it), which pushes players toward gimmicks.
- "Liquid locks" and things like Aerogel exist only because the underlying
  physics is broken enough to need a hack plugged into the gap.
- The game is tuned toward colony failure — dupes die, colonies often
  collapse — as if that were the point.

The fix isn't just "simulate it properly" — it's a specific reskin that
turns real conservation into the actual game mechanic: dupes become
**Gnomes**, small magical creatures whose magic is the *one sanctioned
exception* to an otherwise strictly-conserved simulation. A gnome can create
or remove heat, or mass, but only by spending **Gin** — a limited mana
resource, regenerated by brewing and drinking gin or eating juniper berries
("this is why I wanted such an overbuilt terrarium! It does all have a
vision in the end" — the closed water cycle work traces directly back to
this, not to abstract engineering taste). Brewing/distilling is called out
as core to the whole thing working, not flavour.

Death is redesigned around the same idea: a gnome in danger (lava,
suffocation, cold) teleports to an **ethereal layer** instead of dying —
gas masks and cooling devices as cheap Gin-powered mitigations first, an
ethereal escape as the last resort, rescuable by another gnome. The
"liquids defying gravity in pipes" problem gets the same treatment rather
than being quietly kept: **ethereal pipes** are an explicit, sanctioned
magic shortcut (input/output nodes moving mass outside the physical
simulation, ONI-pipe-like) instead of a physical mechanic pretending not to
break its own rules.

Beyond survival: farming and machine-building as expected pillars, plus two
knowledge-economy pillars that don't exist in ONI at all — gnomes can't
magically duplicate text, so books/letters are hand-copied and shared
between colonies over an expensive "ethereal channel" via a BitTorrent-like
page-at-a-time seeding protocol (scored roughly like Factorio's throughput:
pages per minute), and a "Gnome Grandmother" correspondence mechanic that
occasionally mails back something genuinely useful, at the cost of the
resources spent writing to her.
